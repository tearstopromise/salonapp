...
$sql    = "SELECT * FROM tbl_account WHERE sql_acc_email = '" . $username . "'"; /** checks if profile exists */
$select = mysql_query($sql);
if (mysql_num_rows($select) > 0) {
    $sql    = "SELECT * FROM tbl_account WHERE sql_acc_email = '" . $username . "' AND sql_acc_password = '" . $pass . "'"; /** checks if Password matches with the username given */
    $select = mysql_query($sql);
    if (mysql_num_rows($select) > 0) {
        $verifycode = "v1";
        /** v1 = Profile Verified */
